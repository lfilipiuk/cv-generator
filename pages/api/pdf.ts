...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const html = md
    .render(req.body.text)
    .replace(/&lt;!-- newpage --&gt;/g, '<div class="page-break"></div>');

  // Include CSS styles
  const cssPath = path.join(process.cwd(), "public", "resume.css");
  const cssContent = fs.readFileSync(cssPath, "utf-8");
  const imageHtml = req.body.image
    ? `<div class="photo"><img src="${req.body.image}" alt="Profile Photo"/></div>`
    : "";
  ...
  try {
    const api2pdfResponse = await fetch(
